  ...
  //if messagedlg('Deseja realmente excluir os registros?',mtInformation , mbOKCancel,0) = mrYes then
     ...
     if excluir = 'Vendas' then
      begin
        ...
        if messagedlg('Deseja realmente excluir as vendas de ' + DateToStr(dataInicial.date) +  ' à ' + DateToStr(dataFinal.date) + '?',mtInformation , MbYESNO,0) = mrYes then
        begin
       ...
       if excluir = 'Caixa' then
          begin
                if messagedlg('Deseja realmente excluir os dados de caixas de ' + DateToStr(dataInicial.date) +  ' à ' + DateToStr(dataFinal.date) + '?',mtInformation , MbYESNO,0) = mrYes then
                 begin
       ...
       if excluir = 'Movimentacoes' then
          begin
                if messagedlg('Deseja realmente excluir as Movimentações de ' + DateToStr(dataInicial.date) +  ' à ' + DateToStr(dataFinal.date) + '?',mtInformation , MbYESNO,0) = mrYes then
                  begin
                   dm.query_mov.Close;
